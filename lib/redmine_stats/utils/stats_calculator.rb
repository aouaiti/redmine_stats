  ...
  module Utils
    class StatsCalculator
      ...
      def self.calculate_project_health_score(project, options={})
        ...
        # Get basic metrics
        total_issues = project.issues.created_between(from, to).count
        closed_issues = project.issues.closed.where('issues.updated_on BETWEEN ? AND ?', from, to).count
        overdue_issues = project.issues.open.where('due_date < ? AND due_date IS NOT NULL', Date.today).count
        avg_resolution_time = project.average_resolution_time(from: from, to: to)
        
        # Calculate score components
        resolution_rate = total_issues > 0 ? (closed_issues.to_f / total_issues * 100) : 100
        overdue_penalty = total_issues > 0 ? (overdue_issues.to_f / total_issues * 100) : 0
        time_factor = avg_resolution_time > 0 ? [100 - (avg_resolution_time / 86400.0 * 10), 0].max : 100
        ...
        trends = periods.map do |period|
          {
            ...
            score: user.contribution_score(project, period)
          }
        end
      ...
      def self.calculate_active_periods(project, options={})
        ...
        # Get all journals (represents activity)
        journals = Journal.joins(:issue)
      ...
      def self.health_score_formula_description
        {
          ...
          time_factor: {
            ...
            # Return a description of how contribution scores are calculated      def self.contribution_score_formula_description        {          administrative: {            description: "Sum of all administrative tasks",            components: {              issues_created: {                description: "Issues created by the user",                weight: 1.0              },              issues_assigned: {                description: "Issues assigned to the user",                weight: 1.0              },              issues_closed: {                description: "Issues closed by the user",                weight: 1.0              },              comments: {                description: "Comments added by the user",                weight: 1.0              },              status_updates: {                description: "Status updates made by the user",                weight: 1.0              }            }          },          technical: {            description: "Technical contribution based on resolution timing",            components: {              on_time_resolutions: {                description: "Issues resolved before due date",                weight: 3.0              },              not_on_time_resolutions: {                description: "Issues resolved after due date",                weight: 1.0              }            }          }        }      end
    end
  end
end 